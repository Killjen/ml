...
def tfidf(training_data, test_data):
    ...
    train_vecs = tfidf_transformer.fit_transform(training_data)
    test_vecs = tfidf_transformer.transform(test_data)
...
#   Test without tfidf data

    mnb = MultinomialNB()
    print "Multinomial NB no tfidf: "
    test_class(mnb)
    print_top15(featureList, mnb)


    opt_mnb = MultinomialNB()
    opt_mnb = param_optimization(mnb, X_train)
    print "Multinomial NB no tfidf opti: "
    test_class(opt_mnb)
    print_top15(featureList, opt_mnb)
#___________________________________________________
#   Test with tfidf data

    X_train, X_test = tfidf(X_train, X_test)

    mnb = MultinomialNB()
    print "Multinomial NB tfidf: "
    test_class(mnb)
    print_top15(featureList, mnb)
    ...
    opt_mnb = MultinomialNB()
    opt_mnb = param_optimization(mnb, X_train)
    print "Multinomial NB tfidf opti: "
    test_class(opt_mnb)
    print_top15(featureList, opt_mnb)